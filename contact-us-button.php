...
function contact_us_button_styles(){
    ...
    list($r, $g, $b) = $mainButtonSettings['contact_us_button_options_main_button_color'] ? sscanf($mainButtonSettings['contact_us_button_options_main_button_color'], "#%02x%02x%02x") : ""; 
   ...
   <style>
        ...
        <?php foreach($buttonsSettings as $button):?>
            .cubp-social-button.<?php echo $button['contact_us_button_options_name'] ?>:before { background-color: <?php echo $button['contact_us_button_options_color']?>; }
            .cubp-social-button.<?php echo $button['contact_us_button_options_name'] ?> i { color: <?php echo $button['contact_us_button_options_color']?>; }
        <?php endforeach;?>
    </style>;
<?php }
...
function contact_us_button_enqueue_scripts() {
    wp_enqueue_style( 'contact-us-button_font_awesome', plugins_url( 'assets/font-awesome-6.1.1/css/all.min.css' , __FILE__ ));
    wp_enqueue_style( 'contact-us-button_styles', plugins_url( 'assets/css/contact-us-button-styles.css' , __FILE__ ));
    wp_enqueue_script( 'contact-us-button_scripts', plugins_url( 'assets/js/contact-us-button-scripts.js' , __FILE__ ), array( 'jquery' ), '', true );
}
...
function contact_us_button_view(){
     if($buttonsSettings = get_option('buttonsSettings')):
        $mainButtonSettings = get_option('mainButtonSettings');
     ?>
        <div class="cubp-social-buttons">
            ...
            <span class="cubp-first-social-button seo-all-in-one-but" style="background-color:<?php echo !empty($mainButtonSettings['contact_us_button_options_main_button_color']) ? $mainButtonSettings['contact_us_button_options_main_button_color'] : '#4dc147' ?>">
            <?php if($mainButtonSettings['contact_us_button_options_main_button_logo']): ?>
                <?php echo $mainButtonSettings['contact_us_button_options_main_button_logo']; ?>
            <?php else: ?>
